...
def get_with_retry(scraper, url, retries=3):
    """Функция для упорного выбивания данных из Шоппера"""
    for i in range(retries):
        try:
            res = scraper.get(url, impersonate="chrome120", verify=False, timeout=90)
            if res.status_code == 200:
                # Очищаем мусор сразу
                clean = re.sub(r'(?i)<(name|parentid|productscount)>.*?</\1>', '', res.text)
                # Ищем блоки <url>
                found = re.findall(r'(?i)<url\b[^>]*>.*?</url>', clean, re.DOTALL)
                if len(found) > 0:
                    return found
            print(f"  [Попытка {i+1}] Мало данных или ошибка {res.status_code}, жду...")
        except Exception as e:
            print(f"  [Попытка {i+1}] Ошибка связи: {e}")
        time.sleep(3) # Пауза между ударами по серверу
    return []

def main():
    print("--- ЗАПУСК ГЛУБОКОЙ ОЧИСТКИ (С ПОВТОРАМИ) ---")
    scraper = requests.Session()
    
    try:
        # 1. Получаем список всех под-карт
        r = scraper.get(URL, impersonate="chrome120", verify=False, timeout=60)
        sub_maps = re.findall(r'(?i)<loc>(.*?)</loc>', r.text)
        print(f"Всего под-карт в индексе: {len(sub_maps)}")

        all_urls = []
        for index, sub_url in enumerate(sub_maps):
            sub_url = sub_url.strip()
            # Берем товары, категории, новости и инфо
        ...
        for i in range(0, len(all_urls), LIMIT):
            ...
            with open(filename, "w", encoding="utf-8") as f:
                f.write(header + "\n" + "\n".join(chunk) + "\n</urlset>")
            print(f"СОЗДАН ФАЙЛ: {filename} ({len(chunk)} ссылок)")
